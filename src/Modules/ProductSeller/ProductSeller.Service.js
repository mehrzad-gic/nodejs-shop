...
async function storeService(req, res, next) {
    ...
    try {
        ...
        const { slug } = req.params;
        const { branch_id, price, currency_code, description, status } = req.body;
...
async function showService(req, res, next) {
    ...
    try {
        ...
        // product seller
        const productSeller = await query("select ps.*, b.name as branch_name, b.slug as branch_slug, b.image as branch_image, p.name as product_name, p.slug as product_slug, p.image as product_image from product_sellers ps left join branches b on ps.branch_id = b.id left join products p on ps.product_id = p.id where ps.id = $1", [id]);
